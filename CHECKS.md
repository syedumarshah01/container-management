...
`check.bat` (Linux/macOS: `./check.sh`) builds `tools/MoneyChecks` and runs it. Today that is **515
assertions over 79 of the app's 118 service methods**, in 62 named groups, against a throwaway database in
...
| `FreightSplit` | the expenses shared by weight into a piece's cost: shares equal the bills to the paisa, the odd paisa on the biggest lot, and an unweighed item stops the sharing for the box. Also the rate a picked line brings with it - a yen goods line re-saved at the rate kept on it is not re-valued by a rate written later, and one rule decides what the box holds for both tables on the page |
...
| `PutAway` | what closing a container does and does not do: the lot leaves the lists that are only for selling, keeps every money figure it had, stays in the rows the Containers page is built from so hiding it is a choice the page makes rather than a figure that changed, can still be paid while it is shut, comes back identical when it is re-opened, and is called the same word on the lot's own heading, on its printed sheet and in the Reports column. It also holds the two rules that stop a lot being put away wrongly: a lot with stock on it is refused - out loud, naming how many items are in the way - and a put-away lot stays out of the open list's *search* as well as out of its view, with the aside counted over the book so typing cannot change it. It ends on the stock page, where a closed lot's shelf hides behind the same kind of button: an item an open lot also holds stays listed with only its open lots, an item held only by closed lots leaves the list, and every shelf figure - the row's own and the page's total - is the book's either way, so the hidden 4,500 rupees are named as what a sum-of-what-is-listed leaves out rather than quietly uncounted |
