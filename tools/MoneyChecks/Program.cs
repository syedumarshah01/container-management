...
public static class Program
{
    ...
    /// <summary>
    /// A container the shop could not sell: the customers take their goods back, the shop hands the lot on to
    /// the supplier, and whatever comes with it lands where the shop says it landed. What arrived stays what
    /// arrived, the units leave the shelf, and no page is allowed to decide on the shop's behalf whether a
    /// rupee came into the till or settled a bill - which is the whole point of the feature, so it is the
    /// whole point of these checks.
    /// </summary>
    private static async Task GoodsBack(string dir)
    {
        var file = Path.Combine(dir, "returns.db");
        var services = new ServiceCollection();
        services.AddDbContextFactory<AppDbContext>(o => o.UseSqlite($"Data Source={file};Cache=Shared;Mode=ReadWriteCreate"));
        var sp = services.BuildServiceProvider();
        var f = sp.GetRequiredService<IDbContextFactory<AppDbContext>>();
        ...
        var box = await inventory.CreateContainerAsync(
            "RETURN box", "RET-1", "Japan", new DateTime(2026, 4, 2), null, "PKR", 1m);
        var mug = await inventory.AddGoodsAsync(box.Id, "Return mug", "pcs", "RMG-1", 100m, 500m, null, null, null, 0.5m, null);
        var tray = await inventory.AddGoodsAsync(box.Id, "Return tray", "pcs", "RTR-1", 50m, 2_000m, null, null, null, 4m, null);
        await inventory.AddExpenseAsync(box.Id, new DateTime(2026, 4, 3), "Sea Freight", 90_000m, null);
        await inventory.UpdateImportDetailsAsync(box.Id, "Osaka Returns", 100_000m, null, null, null);

        Head("the units leave the shelf, and the bill is left exactly as it was written");
        Eq("what is owed on the lot is its bill less whatever has settled it", 100_000m, await inventory.SupplierBalanceAsync(box.Id));
        Eq("the freight is shared over 250 kg of goods", 250m, (await inventory.GetExpenseSplitAsync(box.Id)).TotalWeightKg);
        var tillBefore = await CashInHandAsync(f);

        var cashBack = await inventory.ReturnToSupplierAsync(
            mug.Id, new DateTime(2026, 4, 10), 40m, "customers sent them back", 25_000m, 0m, 0m, null);
        var mugNow = await ReadItemAsync(f, mug.Id);
        Eq("40 pieces are off the shelf", 60m, mugNow.QuantityRemaining);
        Eq("and what arrived on the container is still what arrived", 100m, mugNow.QuantityReceived);
        Eq("the till took the refund, once, as money in", 25_000m, await CashInHandAsync(f) - tillBefore);
        Eq("a refund does not quietly rewrite the bill the supplier sent", 100_000m, await inventory.SupplierBalanceAsync(box.Id));
        Eq("the divisor is what was landed, so goods going back do not push their freight onto the pieces that stayed",
            250m, (await inventory.GetExpenseSplitAsync(box.Id)).TotalWeightKg);
        await Throws<InvalidOperationException>("a count cannot put goods that left the building back on the shelf",
            async () => await inventory.AdjustStockAsync(mug.Id, 100m, "recount"));
        Eq("a count of what is really here is still believed", 60m, (await ReadItemAsync(f, mug.Id)).QuantityRemaining);
        await Throws<InvalidOperationException>("and nothing can be sent back that is not in the shop",
            async () => await inventory.ReturnToSupplierAsync(mug.Id, new DateTime(2026, 4, 11), 80m, null, 0m, 0m, 0m, null));

        Head("a settlement against their bill moves the lot and never touches the till");
        var till2 = await CashInHandAsync(f);
        var credit = await inventory.ReturnToSupplierAsync(
            tray.Id, new DateTime(2026, 4, 12), 10m, null, 0m, 8_000m, 0m, null);
        Eq("the lot owes that much less", 92_000m, await inventory.SupplierBalanceAsync(box.Id));
        Eq("not one rupee moved in the till, because none did", 0m, await CashInHandAsync(f) - till2);
        Check("a credit note is not a payment the till has to know about",
            !InventoryService.MovesCash(InventoryService.CreditNote) && InventoryService.MovesCash("Cash")
            && InventoryService.MovesCash("TT") && InventoryService.MovesCash(null));
        await using (var db = await f.CreateDbContextAsync())
        {
            var pays = await db.SupplierPayments.AsNoTracking().Where(p => p.ContainerId == box.Id).ToListAsync();
            Check("the line that settled it says what it was, so nobody reads it as money paid out",
                pays.Count == 1 && pays[0].Method == InventoryService.CreditNote && pays[0].Amount == 8_000m,
                string.Join(" / ", pays.Select(p => p.Method + " " + p.Amount)));
        }
        await Throws<InvalidOperationException>("a settlement cannot be bigger than the bill there is to settle",
            async () => await inventory.ReturnToSupplierAsync(tray.Id, new DateTime(2026, 4, 13), 1m, null, 0m, 500_000m, 0m, null));
        Eq("and the refused row wrote nothing at all", 40m, (await ReadItemAsync(f, tray.Id)).QuantityRemaining);

        Head("what the forwarder gives back comes off the bills, and every piece is costed again");
        var billed = (await inventory.GetExpenseSplitAsync(box.Id)).ExpenseTotal;
        var mugCost = (await ReadItemAsync(f, mug.Id)).EffectiveCost;
        var freightBack = await inventory.ReturnToSupplierAsync(
            mug.Id, new DateTime(2026, 4, 14), 5m, null, 0m, 0m, 10_000m, null);
        Eq("a recovery is a bill line the other way, so the pool shrinks by exactly that",
            billed - 10_000m, (await inventory.GetExpenseSplitAsync(box.Id)).ExpenseTotal);
        Check("and the pieces still on the shelf are costed lower than they were",
            (await ReadItemAsync(f, mug.Id)).EffectiveCost < mugCost,
            (await ReadItemAsync(f, mug.Id)).EffectiveCost + " against " + mugCost);
        Eq("the units the freight recovery carried are off the shelf too", 55m, (await ReadItemAsync(f, mug.Id)).QuantityRemaining);
        await Throws<InvalidOperationException>("nothing is recovered against freight beyond what the bills were",
            async () => await inventory.ReturnToSupplierAsync(mug.Id, new DateTime(2026, 4, 15), 1m, null, 0m, 0m, 5_000_000m, null));

        Head("the pages read the lot as one story, and a refund is not a sale");
        var p = await reports.GetContainerProfitAsync(box.Id);
        Eq("what landed is still reported as what landed", 150m, p.QtyReceived);
        Eq("what is left is what is on the shelf", 95m, p.RemainingQty);
        decimal sentTotal;
        await using (var db = await f.CreateDbContextAsync())
        {
            sentTotal = (await db.SupplierReturns.AsNoTracking().Where(r => r.ContainerId == box.Id).ToListAsync())
                .Sum(r => r.Quantity);
        }
        Eq("the goods line and the returns are the same units added once", sentTotal, p.QtyReceived - p.RemainingQty);
        Eq("nothing sold, so the lot reports no revenue at all", 0m, p.Revenue);
        Eq("a refund into the till is not a sale on the lot", 0m, p.Profit);
        Eq("the bills on the lot are the freight less what was recovered against it", 80_000m, p.Expenses);

        Head("undoing a return puts back every figure it wrote, and only those");
        await inventory.RemoveReturnAsync(credit.Id);
        Eq("the units are back on the shelf", 50m, (await ReadItemAsync(f, tray.Id)).QuantityRemaining);
        Eq("the bill stands where it did before the settlement", 100_000m, await inventory.SupplierBalanceAsync(box.Id));
        var till3 = await CashInHandAsync(f);
        await inventory.RemoveReturnAsync(cashBack.Id);
        Eq("taking a refund back out of the book moves the till by exactly the refund", -25_000m,
            await CashInHandAsync(f) - till3);
        await inventory.RemoveReturnAsync(freightBack.Id);
        Eq("the freight pool is rebuilt from the bills that are left", 90_000m, (await inventory.GetExpenseSplitAsync(box.Id)).ExpenseTotal);
        Eq("and a piece costed again from those bills comes back to the cost it had", mugCost,
            (await ReadItemAsync(f, mug.Id)).EffectiveCost);
        await using (var db = await f.CreateDbContextAsync())
        {
            var left = await db.SupplierReturns.AsNoTracking().Where(r => r.ContainerId == box.Id).ToListAsync();
            Check("every line of the undone returns is gone, and the shelf is whole again",
                left.Count == 0 && (await ReadItemAsync(f, mug.Id)).QuantityRemaining == 100m, "left: " + left.Count);
        }

        Head("a container may be deleted only when nothing in the book speaks of it");
        var spare = await inventory.CreateContainerAsync(
            "SPARE box", "SPARE", "Japan", new DateTime(2026, 4, 20), null, "PKR", 1m);
        await inventory.AddGoodsAsync(spare.Id, "Spare mug", "pcs", null, 5m, 100m, null, null, null, null, null);
        await inventory.DeleteContainerAsync(spare.Id);
        Check("a lot nothing was written against goes, and it goes for real",
            !(await inventory.ListContainersAsync()).Any(c => c.Id == spare.Id));
        await using (var db = await f.CreateDbContextAsync())
        {
            Check("its goods line is taken with it rather than left pointing at nothing",
                !await db.ContainerItems.AnyAsync(i => i.ContainerId == spare.Id));
        }
        await Throws<InvalidOperationException>("a lot whose goods came back from the supplier stays in the book",
            async () => await inventory.DeleteContainerAsync(box.Id));

        var sold = await inventory.CreateContainerAsync(
            "SOLD box", "SOLD-1", "Japan", new DateTime(2026, 4, 21), null, "PKR", 1m);
        var soldMug = await inventory.AddGoodsAsync(sold.Id, "Sold mug", "pcs", null, 10m, 100m, null, null, null, 1m, null);
        var buyer = await ledger.CreateCustomerAsync("Return buyer", null, null, null);
        var bill = await sales.CreateSaleAsync(buyer.Id, new DateTime(2026, 4, 22), new List<NewSaleLineInput>
        {
            new() { ContainerId = sold.Id, ContainerItemId = soldMug.Id, ProductId = soldMug.ProductId, ProductName = "Sold mug", Unit = "pcs", Quantity = 3m, UnitPrice = 300m }
        }, 0m, "Cash", null, 0m, null);
        await sales.CancelSaleAsync(bill.Id);
        Eq("a cancelled invoice puts its units back on the shelf", 10m, (await ReadItemAsync(f, soldMug.Id)).QuantityRemaining);
        await Throws<InvalidOperationException>("but the lot cannot be deleted for it: the invoice is still on the record",
            async () => await inventory.DeleteContainerAsync(sold.Id));
        await inventory.SetStatusAsync(sold.Id, ContainerStatus.Closed);
        Check("closing it is what takes it off the working pages, stock and all",
            !(await inventory.GetSellableStockAsync()).Any(o => o.ContainerId == sold.Id)
            && !(await inventory.ContainersWithStockAsync()).Any(c => c.Id == sold.Id));
    }

    // ------------------------------------------------------------------ taking a bill out

    /// <summary>
    /// The door a cancelled bill needs: taken out of the book so the container behind it can be put away,
    /// with a number that outlives it so no two customers ever hold the same paper. What the shop paid and
    /// what it took back are money that moved, so those stay whether or not the bill does.
    /// </summary>
    private static async Task BillRemoval(string dir)
    {
        var file = Path.Combine(dir, "bills.db");
        var services = new ServiceCollection();
        services.AddDbContextFactory<AppDbContext>(o => o.UseSqlite($"Data Source={file};Cache=Shared;Mode=ReadWriteCreate"));
        var sp = services.BuildServiceProvider();
        var f = sp.GetRequiredService<IDbContextFactory<AppDbContext>>();
        ...
        var box = await inventory.CreateContainerAsync("BILL box", "BILL-1", "Japan", new DateTime(2026, 5, 2), null, "PKR", 1m);
        var mug = await inventory.AddGoodsAsync(box.Id, "Bill mug", "pcs", null, 10m, 100m, null, null, null, null, null);
        var customer = await ledger.CreateCustomerAsync("Bill buyer", null, null, null);

        Head("a bill's number is its own, and it does not come back around");
        var b1 = await Bill(customer, sales, box, mug, 4m, 500m, new DateTime(2026, 5, 3));
        var b2 = await Bill(customer, sales, box, mug, 3m, 500m, new DateTime(2026, 5, 3));
        Eq("the first bill in a fresh book is numbered 1", 1m, b1.InvoiceNo);
        Eq("and the next is 2, whatever its row is", 2m, b2.InvoiceNo);
        await sales.CancelSaleAsync(b2.Id);
        Eq("the cancellation names the same number on the line it writes", 2m, await CancelNumberAsync(f, b2.Id));
        await Throws<InvalidOperationException>("a bill that stands cannot be taken out",
            async () => await sales.DeleteCancelledSaleAsync(b1.Id));
        Check("a cancelled bill with nothing paid on it can be", await sales.BillCanBeDeletedAsync(b2.Id));

        var before = await sales.RemainingOnInvoiceAsync(b1.Id);
        var tillBefore = await CashInHandAsync(f);
        var balanceBefore = await LedgerBalanceAsync(f, customer.Id);
        await sales.DeleteCancelledSaleAsync(b2.Id);
        Eq("and taking it out moves no money on the bill that stays", before, await sales.RemainingOnInvoiceAsync(b1.Id));
        Eq("nothing in the till", 0m, await CashInHandAsync(f) - tillBefore);
        Eq("nothing on the customer's balance", balanceBefore, await LedgerBalanceAsync(f, customer.Id));
        Eq("the units it had borrowed are not put back a second time, either", 6m, (await ReadItemAsync(f, mug.Id)).QuantityRemaining);
        await using (var db = await f.CreateDbContextAsync())
        {
            var kept = await db.LedgerEntries.AsNoTracking()
                .Where(x => x.CustomerId == customer.Id
                            && (x.Description == $"Sale #{b2.InvoiceNo} to Bill buyer"
                                || x.Description == $"Cancelled sale #{b2.InvoiceNo}"))
                .ToListAsync();
            var rowStillThere = await SaleRowExistsAsync(f, b2.Id);
            Check("the bill's two ledger lines stay in the book, naming the number its paper carried, and no "
                + "longer point at a row the next sale will be handed",
                kept.Count == 2 && kept.All(x => x.SaleId is null) && !rowStillThere,
                $"{kept.Count} lines, {kept.Count(x => x.SaleId is null)} unlinked, row still there: {rowStillThere}");
        }

        var b3 = await Bill(customer, sales, box, mug, 2m, 500m, new DateTime(2026, 5, 4));
        Eq("the next bill does not get the number that was just freed", 3m, b3.InvoiceNo);
        Check("even though it did get the row that was freed", b3.Id == b2.Id, $"b2 {b2.Id}, b3 {b3.Id}");
        Eq("and the series remembers how far it has gone", 3m, await SeriesAsync(f));

        Head("a bill money moved on stays in the book, and so does one its goods came back on");
        var b4 = await Bill(customer, sales, box, mug, 1m, 500m, new DateTime(2026, 5, 5), paid: 500m);
        await sales.CancelSaleAsync(b4.Id);
        Check("refunded through the till, so it cannot be taken out", !await sales.BillCanBeDeletedAsync(b4.Id));
        await Throws<InvalidOperationException>("and it says so rather than failing quietly",
            async () => await sales.DeleteCancelledSaleAsync(b4.Id));
        Eq("the till still holds the money that went in and came back out", 0m, await CashInHandAsync(f) - tillBefore);

        var b5 = await Bill(customer, sales, box, mug, 1m, 500m, new DateTime(2026, 5, 6));
        var line = await FirstLineAsync(f, b5.Id);
        await sales.ReturnItemsAsync(b5.Id, new List<SaleReturnInput> { new() { SaleLineId = line.Id, Quantity = 1m } });
        Check("a bill whose goods all came back is a sale that happened, so it stays", !await sales.BillCanBeDeletedAsync(b5.Id));

        Head("which is how a container that sold everything gets put away");
        await sales.CancelSaleAsync(b1.Id);
        await sales.DeleteCancelledSaleAsync(b1.Id);
        await sales.CancelSaleAsync(b3.Id);
        await sales.DeleteCancelledSaleAsync(b3.Id);
        Eq("b4 and b5, whose money and goods stayed put, are still on the customer", 2m,
            await BillCountAsync(f, customer.Id));
        await Throws<InvalidOperationException>("and a lot whose remaining bills cannot be taken out keeps their "
            + "lines, so it stays too",
            async () => await inventory.DeleteContainerAsync(box.Id));

        var spare = await inventory.CreateContainerAsync("SPARE box", "BILL-2", "Japan", new DateTime(2026, 5, 7),
            null, "PKR", 1m);
        var spareItem = await inventory.AddGoodsAsync(spare.Id, "Bill mug", "pcs", null, 2m, 100m, null, null, null, null, null);
        var only = await Bill(customer, sales, spare, spareItem, 2m, 500m, new DateTime(2026, 5, 7));
        await Throws<InvalidOperationException>("a lot sold out on one bill that stands is not deletable",
            async () => await inventory.DeleteContainerAsync(spare.Id));
        await sales.CancelSaleAsync(only.Id);
        await Throws<InvalidOperationException>("not even once that bill is cancelled, while it is still in the book",
            async () => await inventory.DeleteContainerAsync(spare.Id));
        await sales.DeleteCancelledSaleAsync(only.Id);
        await inventory.DeleteContainerAsync(spare.Id);
        Check("take the bill out and the lot goes with it",
            !(await inventory.ListContainersAsync()).Any(c => c.Id == spare.Id));
        await using (var db = await f.CreateDbContextAsync())
        {
            Check("with nothing left pointing at the goods from a lot that is gone",
                !await db.ContainerItems.AnyAsync(i => i.ContainerId == spare.Id)
                && !await db.SaleLines.AnyAsync(l => l.ContainerId == spare.Id));
        }

        // Named Bill rather than Sale, and its parameters named clear of the locals above: a local function
        // shares its enclosing method's declaration space, and reusing a name there is a compiler error rather
        // than a shadow.
        static async Task<Sale> Bill(Customer c, SalesService s, CargoContainer container, ContainerItem goods,
            decimal qty, decimal price, DateTime date, decimal paid = 0m) =>
            await s.CreateSaleAsync(c.Id, date, new List<NewSaleLineInput>
            {
                new() { ContainerId = container.Id, ContainerItemId = goods.Id, ProductId = goods.ProductId, ProductName = "Bill mug", Unit = "pcs", Quantity = qty, UnitPrice = price }
            }, paid, "Cash", null, 0m, null);
    }

    private static async Task<int> CancelNumberAsync(IDbContextFactory<AppDbContext> factory, int saleId)
    {
        await using var db = await factory.CreateDbContextAsync();
        var rows = await db.LedgerEntries.AsNoTracking().Where(x => x.SaleId == saleId).ToListAsync();
        var text = rows.FirstOrDefault(x => x.Description.StartsWith("Cancelled sale #"))?.Description ?? "";
        return int.TryParse(text.Replace("Cancelled sale #", ""), out var no) ? no : -1;
    }

    private static async Task<bool> SaleRowExistsAsync(IDbContextFactory<AppDbContext> factory, int saleId)
    {
        await using var db = await factory.CreateDbContextAsync();
        return await db.Sales.AnyAsync(x => x.Id == saleId);
    }

    private static async Task<decimal> LedgerBalanceAsync(IDbContextFactory<AppDbContext> factory, int customerId)
    {
        await using var db = await factory.CreateDbContextAsync();
        var rows = await db.LedgerEntries.AsNoTracking().Where(x => x.CustomerId == customerId).ToListAsync();
        return Money.Round(rows.Sum(x => x.Debit - x.Credit));
    }

    private static async Task<decimal> SeriesAsync(IDbContextFactory<AppDbContext> factory)
    {
        await using var db = await factory.CreateDbContextAsync();
        var row = await db.NumberSeries.AsNoTracking().FirstOrDefaultAsync(x => x.Name == "invoice");
        return row?.LastIssued ?? -1m;
    }

    private static async Task<SaleLine> FirstLineAsync(IDbContextFactory<AppDbContext> factory, int saleId)
    {
        await using var db = await factory.CreateDbContextAsync();
        return await db.SaleLines.FirstAsync(x => x.SaleId == saleId);
    }

    private static async Task<int> BillCountAsync(IDbContextFactory<AppDbContext> factory, int customerId)
    {
        await using var db = await factory.CreateDbContextAsync();
        return await db.Sales.CountAsync(x => x.CustomerId == customerId);
    }
